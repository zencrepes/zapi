...
import TestingStates from './testingStates.type';
import TestingStatesData from './data/data.type';
import TestingStatesConfig from './config/config.type';
...
@Resolver(TestingStates)
export default class TestingStatesResolver {
  ...
  public async geTestingStates(): Promise<TestingStates> {
    return new TestingStates();
  }

  @ResolveField(() => TestingStatesData, {
    name: 'data',
  ...
  public async getData(
    ...
    @Parent() parent: TestingStates,
  ): Promise<TestingStatesData> {
    const userConfig = this.confService.getUserConfig();

    const data = new TestingStatesData();
    if (query === undefined || query === null) {
      query = JSON.stringify({});
    }
    data.query = query;
    data.esQuery = await getEsQuery(query);
    data.count = await this.countService.getCount(query, userConfig.elasticsearch.dataIndices.testingStates + '*');

    return data;
  }

  @ResolveField(() => TestingStatesConfig, {
    name: 'config',
    description: 'Access to configuration values and metadata',
  })
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  public async getConfig(@Parent() parent: TestingStates): Promise<TestingStatesConfig> {
    return new TestingStatesConfig();
  }
}
