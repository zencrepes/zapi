...
import DataStateConnection from './items/stateConnection.type';
import ItemSortorder from './items/itemSortorder.type';
import DataItemsService from '../../utils/data/items/items.service';

import StatesAggregationConnection from './aggregations/statesAggregationConnection.type';
...
export default class DataResolver {
  ...
  @ResolveField(() => DataStateConnection, {
    name: 'items',
  ...
  public async getItem(
    ...
    const userConfig = this.confService.getUserConfig();
    const item = await this.itemsService.findOneById(id, userConfig.elasticsearch.dataIndices.testingStates);
    return item;
  }

  @ResolveField(() => StatesAggregationConnection, {
    name: 'aggregations',
