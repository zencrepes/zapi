...
import DataMavenPomConnection from './items/mavenPomConnection.type';
import ItemSortorder from './items/itemSortorder.type';
import DataItemsService from '../../utils/data/items/items.service';

import MavenPomsAggregationConnection from './aggregations/mavenPomsAggregationConnection.type';
...
export default class DataResolver {
  ...
  @ResolveField(() => DataMavenPomConnection, {
    name: 'items',
  ...
  public async getItem(
    ...
    const item = await this.itemsService.findOneById(id, userConfig.elasticsearch.dataIndices.githubMavenPoms);
    return item;
  }

  @ResolveField(() => MavenPomsAggregationConnection, {
    name: 'aggregations',
