...
export class EsClientService {
  ...
  constructor(configService: ConfService) {
    ...
    if (
      ...
      this.logger.log('Elasticsearch client initialized with SSL connection to host: ' + userConfig.elasticsearch.host);
    } else {
      this.esClient = new Client({
        node: userConfig.elasticsearch.host,
      });
      this.logger.log('Elasticsearch client initialized with host: ' + userConfig.elasticsearch.host);
    }
  }
