...
import BambooRuns from './bambooRuns.type';
import BambooRunsData from './data/data.type';
import BambooRunsConfig from './config/config.type';
...
@Resolver(BambooRuns)
export default class BambooRunsResolver {
  ...
  public async geBambooRuns(): Promise<BambooRuns> {
    return new BambooRuns();
  }

  @ResolveField(() => BambooRunsData, {
    name: 'data',
  ...
  public async getData(
    ...
    @Parent() parent: BambooRuns,
  ): Promise<BambooRunsData> {
    const userConfig = this.confService.getUserConfig();

    const data = new BambooRunsData();
    if (query === undefined || query === null) {
      query = JSON.stringify({});
    }
    data.query = query;
    data.esQuery = await getEsQuery(query);
    data.count = await this.countService.getCount(query, userConfig.elasticsearch.dataIndices.bambooRuns + '*');

    return data;
  }

  @ResolveField(() => BambooRunsConfig, {
    name: 'config',
    description: 'Access to configuration values and metadata',
  })
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  public async getConfig(@Parent() parent: BambooRuns): Promise<BambooRunsConfig> {
    return new BambooRunsConfig();
  }
}
